...
def classify_cell(
    ...
    near, near_conf = _match_by_hamming(binary_pattern, max_distance=1)
    if near and _geometric_confidence(binary_pattern, intensity_vector) >= 0.55:
        conf = _blend_confidence(near_conf, binary_pattern, near, intensity_vector)
        return near, pattern_to_display_char(near), conf
